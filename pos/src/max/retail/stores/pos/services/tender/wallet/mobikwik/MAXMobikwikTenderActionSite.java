 ...
 *	
 *  Rev 1.0     Oct 14, 2017        Bhanu Priya     Changes done for Mobikwik Integration FES
...
public class MAXMobikwikTenderActionSite extends PosSiteActionAdapter
{
	...
	public void arrive(BusIfc bus)
	{
		 ...
		 boolean isReentryMode = cargo.getRegister().getWorkstation().isTransReentryMode();
	        ...
	        tenderAttributes.put(TenderConstants.TENDER_TYPE,MAXTenderTypeEnum.MOBIKWIK);
	       if(isReentryMode){
	       ...
	       {
	    	    tenderAttributes.put(MAXTenderConstants.WALLET_ORDERID, cargo.getMobikwikResp().getOrderId());
		        tenderAttributes.put(MAXTenderConstants.WALLET_TRANSACTIONID, cargo.getMobikwikResp().getWalletTxnId());
				tenderAttributes.put(TenderConstants.AUTH_CODE, cargo.getMobikwikResp().getWalletTxnId());
	       }
	       
	        MAXTenderMobikwikADO mobikwikADO = null;
	        if(cargo.getTenderADO()==null)
	        {
	        	try
	            {
	                TenderFactoryIfc factory = (TenderFactoryIfc)ADOFactoryComplex.getFactory("factory.tender");
	                mobikwikADO = (MAXTenderMobikwikADO)factory.createTender(tenderAttributes);
	            }
	        ...
	        {
	        	mobikwikADO = (MAXTenderMobikwikADO)cargo.getTenderADO();
	        }
	        try
	        {
	            cargo.getCurrentTransactionADO().addTender(mobikwikADO);
	            cargo.setLineDisplayTender(mobikwikADO);
	            ...
	            registerJournal.journal(mobikwikADO, JournalFamilyEnum.TENDER, JournalActionEnum.ADD);
	      ...
	      //   boolean reEntry=   cargo.getRegister().getWorkstation().isTransReentryMode();
	            ...
	            {
	            	  bus.mail(new Letter("MobikwikReentry"), BusIfc.CURRENT);
	            }
	            else{
	            bus.mail(new Letter("Success"), BusIfc.CURRENT);
	        }
	        ...
	        {
	            assert(false) : "This should never happen, because UI enforces proper format";
	            cargo.setTenderADO(mobikwikADO);
	        }
		}
