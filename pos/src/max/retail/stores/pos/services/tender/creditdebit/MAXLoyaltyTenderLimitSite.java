...
*/
public class MAXLoyaltyTenderLimitSite extends PosSiteActionAdapter
{
	/**
	 * 
	 */
	   ...
	   {	
		...
		catch (TenderException te) 
		{
			...
			try 
			{
				...
				if (responseMap != null)
				{
					if (!("00").equals(responseMap.get("HostResponseCode").toString())) 
					{
...
//						String acqName = responseMap.get("SelectedAquirerName").toString();
					...
					//	String reqBankName = "L-"+acqName;
						...
						//showDialogBoxMethod(responseMap, bus, CommonLetterIfc.NEXT); // Used to show approved status
