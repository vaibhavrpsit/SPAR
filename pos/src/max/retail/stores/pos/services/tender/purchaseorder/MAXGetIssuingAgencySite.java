...
**/
//--------------------------------------------------------------------------
public class MAXGetIssuingAgencySite extends PosSiteActionAdapter
{
    /** revision number **/
    ...
    **/
    ...
    public void arrive(BusIfc bus)
    {
        ...
        try
        {
            ...
            {
                list = ((EnumeratedListValidator)agencyList.getValidator()).getAllowableValues();
            }

            // create list of agencies
            ArrayList aList = new ArrayList(Arrays.asList(list));
    ...
    **/
    ...
    public void depart(BusIfc bus)
    {
        ...
        if (letter.getName().equals("Next"))
        {
            ...
            RetailTransactionADOIfc trxnADO = cargo.getCurrentTransactionADO();
            TenderableTransactionIfc trx = (TenderableTransactionIfc) trxnADO.toLegacy();
            TransactionTotalsIfc totals = trx.getTransactionTotals();
            if(totals.getInclusiveTaxTotal().compareTo(DomainGateway.getBaseCurrencyInstance("0.00"))==0)
            {
            	cargo.getTenderAttributes().put(TenderConstants.TRANSACTION_NON_TAXABLE, new Boolean(true));
            }
            else
            {
            	cargo.getTenderAttributes().put(TenderConstants.TRANSACTION_NON_TAXABLE, new Boolean(false));
            }
            //// make the transation non-taxable
            //scargo.getTenderAttributes().put(TenderConstants.TRANSACTION_NON_TAXABLE, new Boolean(true));  
        }   
    }
}
