...
public class MAXAmazonPayTenderActionSite extends PosSiteActionAdapter {
	...
	public void arrive(BusIfc bus) {
		...
		DataInputBeanModel model = (DataInputBeanModel) uiManager
				.getModel(MAXPOSUIManagerIfc.ENTER_AMAZON_PAY_MOBILE_NUMBER_SCREEN);
		model.getValueAsString("MobileNumberField");
		 /*Rev	2.0  Kumar Vaibhav start*/
		model.getValueAsString("BarcodeField");
		/*end*/
		...
		tenderAttributes.put(TenderConstants.NUMBER,
				model.getValueAsString("MobileNumberField"));
		 /*Rev	2.0  Kumar Vaibhav start*/
		tenderAttributes.put(TenderConstants.NUMBER,
				model.getValueAsString("BarcodeField"));
		...
		tenderAttributes.put(TenderConstants.TENDER_TYPE,
				MAXTenderTypeEnum.AMAZON_PAY);
		/*tenderAttributes.put(TenderConstants.TENDER_TYPE,
				"AmazonPay");*/
		
		if (isReentryMode) {
			tenderAttributes.put(MAXTenderConstants.WALLET_ORDERID,
					REENTRY_WALLETID);
			tenderAttributes.put(MAXTenderConstants.WALLET_TRANSACTIONID,
					model.getValueAsString("TotpCodeField"));
			tenderAttributes.put(TenderConstants.AUTH_CODE,
					model.getValueAsString("TotpCodeField"));
		} else {
			tenderAttributes.put(MAXTenderConstants.WALLET_ORDERID, cargo
					.getAmazonPayResp().getOrderId());
			tenderAttributes.put(TenderConstants.AUTH_CODE, cargo
					.getAmazonPayResp().getAmazonTransactionId());
		}

		MAXTenderAmazonPayADO amazonPayADO = null;
		if (cargo.getTenderADO() == null) {
			try {
				TenderFactoryIfc factory = (TenderFactoryIfc) ADOFactoryComplex
						.getFactory("factory.tender");
				amazonPayADO = (MAXTenderAmazonPayADO) factory
			...
			amazonPayADO = (MAXTenderAmazonPayADO) cargo.getTenderADO();
		}
		try {
			cargo.getCurrentTransactionADO().addTender(amazonPayADO);
			cargo.setLineDisplayTender(amazonPayADO);
			...
			try {
				RegisterJournalIfc registerJournal = jrnlFact
						.getRegisterJournal();
				registerJournal.journal(amazonPayADO, JournalFamilyEnum.TENDER,
			...
			if (cargo.getRegister().getWorkstation().isTransReentryMode()
				...
				bus.mail(new Letter("AmazonPayReEntry"), BusIfc.CURRENT);
			} else {
				bus.mail(new Letter("Success"), BusIfc.CURRENT);
			}
		} catch (TenderException e) {
			assert (false) : "This should never happen, because UI enforces proper format";
			cargo.setTenderADO(amazonPayADO);
		}
	}

	protected void journalTaxStatus(BusIfc bus, HashMap tenderAttributes) {
		String status = (String) tenderAttributes
				.get(TenderConstants.TAXABLE_STATUS);
		if (status != null
				&& status.equals(PurchaseOrderLimitActionSite.TAX_EXEMPT)) {
