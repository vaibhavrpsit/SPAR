 ...
 *
 *	Rev 1.0	  08 July 2019			Mohan Yadav				Changes for Innoviti GooglePay
 * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * ***************************/
...
public class MAXPineLabCreditPhonePeSite extends PosSiteActionAdapter {
	...
	public void arrive(BusIfc bus) {
		...
		try {
			 String mobile=null;
				POSBaseBeanModel model = (POSBaseBeanModel)ui.getModel(MAXPOSUIManagerIfc.ENTER_PHONEPE_NUMBER);
		        PromptAndResponseModel prModel= model.getPromptAndResponseModel();
		        mobile = prModel.getResponseText();

				POSBaseBeanModel beanModel = new POSBaseBeanModel();
				ui.showScreen(MAXPOSUIManagerIfc.EDC_PHONEPE_SCREEN, beanModel);
			/*
			 * if(cargo.getCustomer() instanceof MAXCustomerIfc && cargo.getCustomer()
			 * !=null){ MAXCustomerIfc customer= (MAXCustomerIfc) cargo.getCustomer();
			 * String phoneList = customer.getLoyaltyCustomerPhone() != null ?
			 * customer.getLoyaltyCustomerPhone().getPhoneNumber() : null; if (phoneList !=
			 * null && !phoneList.isEmpty()) { mobile =
			 * customer.getLoyaltyCustomerPhone().getPhoneNumber(); } if(mobile == null ||
			 * mobile.equals("")) { List<PhoneIfc> phoneLst = customer.getContact() != null
			 * ? customer.getContact().getPhoneList() : null; if (phoneLst != null &&
			 * !phoneLst.isEmpty()) { mobile = phoneLst.get(0).getPhoneNumber(); } } }
			 */
			String cashierID="0";
			...
			String txnNumber="";
			if(cashierID !=null && mobile !=null) {
				txnNumber=cargo.getCurrentTransactionADO().getTransactionID().concat("!").concat(cashierID.trim()).concat("!").concat(mobile.trim()).concat("!");
			}else {
				txnNumber=cargo.getCurrentTransactionADO().getTransactionID();
			}
			//responseMap = edcObj.makePostVoidEDC(txnNumber, amountString,invoiceNumber, transactionTime, "00", "18");		
			responseMap = edcObj.doSaleTransaction(cargo.getCurrentTransactionADO().getTransactionID(),
					amountString, invoiceNumber, transactionTime, "0", "0",MAXPineLabTransactionConstantsIfc.PLUTUS_PHONEPE_TRANSACTION_TYPE,null);
			...
			if (responseMap != null && responseMap.size() > 0 ) {
				...
				if (hostResponseCode.equals("APPROVED")) {
					...
					//System.out.println("CardNumber : "+responseMap.get("CardNumber"));
					//System.out.println("HostResponseApprovalCode : "+responseMap.get("HostResponseApprovalCode"));
					//System.out.println("HostResponseRetrievelRefNumber : "+responseMap.get("HostResponseRetrievelRefNumber"));
					if(responseMap.get("HostResponseApprovalCode") != null) {
					tenderAttributes.put(MAXTenderConstants.AUTH_CODE,responseMap.get("HostResponseApprovalCode").toString());
					}else {
						tenderAttributes.put(MAXTenderConstants.AUTH_CODE,"00");
					}
					...
					try {
						...
						// MAX Change for Rev 1.1: Start
						if(schemeType!=null && schemeType.equalsIgnoreCase("PhonePe") && responseMap.get("HostResponseRetrievelRefNumber")!=null){
							
							tenderCharge.setRetrievalReferenceNumber(responseMap.get("HostResponseRetrievelRefNumber").toString());
						}
						//tenderCharge.setGpayNumber(mobile);
						// MAX Change for Rev 1.1: End
					...
					try {
						  ...
						  responseMap = edcObj.doGetStatusTransaction(cargo.getCurrentTransactionADO().getTransactionID(),
									amountString, invoiceNumber, transactionTime, "0", "0",MAXPineLabTransactionConstantsIfc.PLUTUS_PHONEPE_GETSTATUS);
							if(responseMap == null || responseMap.isEmpty()){
								showDialogBoxMethod(responseMap, bus, "OnlinePhonePe");
							}
							else{
								...
								if (hostResponseCode.equals("APPROVED")) {
									HashMap bankNameStrings = getBankNameStrings(bus);
									String acqName = responseMap.get("SelectedAquirerName").toString();
									//String aquirerStatus=responseMap.get("SelectedAquirerStatus").toString();
									String bankCode = getBankCodeFromCardType(bankNameStrings, acqName);
									// MAX Change for Rev 1.1: Start
									String schemeType = null;
									if(responseMap.get("SchemeType")!=null)
