 ...
 * Copyright (c) 2013 Max Hypermarket.    All Rights Reserved. 
  * Rev 1.2	 5th Oct 2019		Karni Singh		POS REQ: Register CRM customer with OTP.
...
public class MAXProcessTICRequestSite extends PosSiteActionAdapter {
	...
	private static Logger logger = Logger
			.getLogger(max.retail.stores.pos.services.sale.MAXProcessTICRequestSite.class);
	...
	//	showProcessCRMRequestScreen(bus);
		...
		//String response = executePost(URL, urlParameters); karni Singh
	...
	protected String createURL(HashMap requestInfo, BusIfc bus) {
		...
		try {
			...
			Logger.getLogger(MAXProcessTICRequestSite.class.getName()).log(null, e);
			return null;
		}
	}
	...
	protected void processSuccessResponse(BusIfc bus, String response, MAXSaleCargoIfc cargo) {
		...
		if(response.equalsIgnoreCase("Timeout")){
			...
			if(responseStatus.trim().equalsIgnoreCase("SUCCESS")){
				...
				if(errorCode==null || errorCode.trim().equalsIgnoreCase("") || errorCode.trim().equalsIgnoreCase("NULL")){
					...
					MAXCustomer customer=null;
					MAXSaleReturnTransaction returnTransaction=null;
					if(cargo.getTransaction()!=null && cargo.getTransaction() instanceof MAXSaleReturnTransaction){
						returnTransaction =(MAXSaleReturnTransaction)cargo.getTransaction();
					...
					else if(cargo.getTransaction()!=null && cargo.getTransaction().getCustomer()!=null){
			...
			/*bus.mail(new Letter("TICSuccess"), BusIfc.CURRENT);*/
	...
	private HashMap populateHashMap(MAXSaleCargoIfc cargo, HashMap requestAttributes) {
	...
	protected void displayErrorDialog(BusIfc bus) {
		...
		beanModel.setResourceID("CRMConnectionErrorSale");
		beanModel.setType(DialogScreensIfc.ERROR);
		//  beanModel.setButtonLetter(DialogScreensIfc.BUTTON_CONTINUE, "Failure");
