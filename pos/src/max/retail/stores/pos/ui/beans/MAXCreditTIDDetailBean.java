...
public class MAXCreditTIDDetailBean extends BaseBeanAdapter {
	...
	protected String columnNames[] = { "TID", "Batch ID", "Amount" };
	protected String dataValues[][] = null;
	
	
	protected MAXCreditTIDDetailBeanModel beanModel = null;
	...
	protected void initializeDataValues()
	{
		...
		dataValues = new String[beanModel.tid.length][3];
		for(int i=0;i<beanModel.tid.length;i++)
		{
			dataValues[i][0]	= beanModel.getTid()[i];
			dataValues[i][1]	= beanModel.getBatchid()[i];  
			dataValues[i][2] 	= beanModel.getAmount()[i].getStringValue();
		}
	...
	public void setModel(UIModelIfc model)
    {
        ...
        {
            if (model instanceof MAXCreditTIDDetailBeanModel)
            {
                beanModel = (MAXCreditTIDDetailBeanModel)model;
                remove();
                removeAll();
        	    buildScreen();
            }
