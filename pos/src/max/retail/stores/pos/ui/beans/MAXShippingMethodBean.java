 ...
 */
// ----------------------------------------------------------------------------
public class MAXShippingMethodBean extends ValidatingBean {
	/**
	 * revision number
	 **/
	...
	// The bean model
	protected MAXShippingMethodBeanModel beanModel = null;
	...
	protected static String EXPECTED_DELIVERY_DATE_FORMAT_LABEL = "ExpectedDeliveryDateFormatLabel";
	protected static String EXPECTED_DELIVERY_TIME_FORMAT_LABEL = "ExpectedDeliveryTimeFormatLabel";
	...
	protected void initializeLabels() {
		...
		shipChargeLabel = uiFactory.createLabel("shipChargeLabel", null,
				UI_LABEL);

		expectedDeliveryDateLabel = uiFactory.createLabel(
				"expectedDeliveryDateLabel", null, UI_LABEL);

		expectedDeliveryTimeLabel = uiFactory.createLabel(
				"expectedDeliveryTimeLabel", null, UI_LABEL);
	}
	...
	protected void initializeFields() {
		shipViaScrollPane = uiFactory.createSelectionList("shipViaList",
				"large");
		shipViaList = (JList) shipViaScrollPane.getViewport().getView();

		shipChargeField = uiFactory.createCurrencyField("shipChargeField",
				"false", "false", "false");
		shipChargeField.setEditable(false);
		expectedDeliveryDateField = uiFactory
				.createEYSDateField("expectedDeliveryDateField");
		expectedDeliveryTimeField = uiFactory
				.createEYSTimeField("expectedDeliveryTimeField");
	}
	...
	protected void layoutComponents() {
		JLabel[] labels = { shipToLabel, emptyLabel, emptyLabel, emptyLabel,
				emptyLabel, emptyLabel, shipViaLabel, shipChargeLabel,
				expectedDeliveryDateLabel, expectedDeliveryTimeLabel };

		JComponent[] components = { nameField, businessName, addressLine1Field,
				addressLine2Field, addressLine3Field, cityStateZipField,
				shipViaScrollPane, shipChargeField, expectedDeliveryDateField,
	...
	public void updateModel() {
		...
		beanModel.setExpectedDeliveryDate(expectedDeliveryDateField.getDate());
		beanModel.setExpectedDeliveryTime(new EYSTime(expectedDeliveryTimeField.getEYSDate())); //Rev 1.1 change
		;
	...
	public void setModel(UIModelIfc model) {
		if (model == null) {
			throw new NullPointerException(
					"Attempt to set ShippingMethodBean model to null");
		} else {
			if (model instanceof MAXShippingMethodBeanModel) {
				beanModel = (MAXShippingMethodBeanModel) model;
				updateBean();
			}
		}
	}
	...
	public void updateBean() {
		...
		nameField.setText(beanModel.getFirstName() + " "
				+ beanModel.getLastName());
		...
		if (beanModel.getCountryInfo() != null) {
			if (beanModel.getStateInfo() != null) {
				cityState = cityState
						+ beanModel.getCountryInfo().getAddressDelimiter()
						+ " " + beanModel.getState();
			}

			cityState = cityState + " " + beanModel.getPostalCode();
			/*if (beanModel.getExtPostalCode() != null
					&& beanModel.getExtPostalCode().length() > 0) {
				cityState = cityState
						+ beanModel.getCountryInfo().getPostalCodeDelimiter()
						+ beanModel.getExtPostalCode();
			}*/
		...
		expectedDeliveryDateField.setDate(beanModel.getExpectedDeliveryDate());
		expectedDeliveryTimeField.setTime(beanModel.getExpectedDeliveryTime());
		setCurrentFocus(shipViaList);
	...
	public void activate() {
		...
		expectedDeliveryDateField.addFocusListener(this);
		expectedDeliveryTimeField.addFocusListener(this);
		updateModel();
	}
	...
	protected void updatePropertyFields() { // begin updatePropertyFields()
		...
		if (expectedDeliveryDateLabel != null) {
			expectedDeliveryDateLabel.setText(retrieveText(
					EXPECTED_DELIVERY_DATE_LABEL,
					expectedDeliveryDateLabel.getText()));
		}
		if (expectedDeliveryTimeLabel != null) {
			expectedDeliveryTimeLabel.setText(retrieveText(
					EXPECTED_DELIVERY_TIME_LABEL,
					expectedDeliveryTimeLabel.getText()));
		}
	...
	public static void main(java.lang.String[] args) {
		...
		MAXShippingMethodBean bean = new MAXShippingMethodBean();

		MAXShippingMethodBeanModel beanModel = new MAXShippingMethodBeanModel();
