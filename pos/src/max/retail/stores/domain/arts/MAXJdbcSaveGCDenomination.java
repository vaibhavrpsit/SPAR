...
public class MAXJdbcSaveGCDenomination extends JdbcUtilities implements MAXARTSDatabaseIfc {

	private static Logger logger = Logger.getLogger(MAXJdbcSaveAcquirerBankDetails.class);

	protected boolean insertGCData(JdbcDataConnection dataConnection, TillIfc till, RegisterIfc register)
			throws DataException {
		FinancialTotalsIfc totals = till.getTotals();
		List dnmList = ((MAXFinancialTotals) totals).getGiftCertificateDenomination();
		if (dnmList != null) {
			for (int i = 0; i < dnmList.size(); i++) {
				MAXDenominationCount count = (MAXDenominationCount) dnmList.get(i);
				if (!insertGCData(dataConnection, register, till, count.getCurrency(), count.getQuantity()))
					return false;
			}
		}
		return true;
	...
	private boolean insertGCData(JdbcDataConnection dataConnection, RegisterIfc register, TillIfc till,
			CurrencyIfc currency, int quantity) throws DataException {

		boolean returnCode = true;

		SQLInsertStatement sql = new SQLInsertStatement();
		sql.setTable(TABLE_GIFT_CERTIFICATE_DETAIL);
		...
		sql.addColumn(FIELD_GIFT_CERTIFICATE_DENOMINATION, getAmount(currency));
		sql.addColumn(FIELD_GIFT_CERTIFICATE_QUANTITY, quantity);
