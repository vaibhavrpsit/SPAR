 ...
 * ===========================================================================
 * $Header: rgbustores/modules/domain/src/oracle/retail/stores/domain/arts/JdbcReadLayawayDescriptions.java /rgbustores_13.4x_generic_branch/1 2011/05/04 11:49:02 mszekely Exp $
 * ===========================================================================
 * NOTES
 ...
 * Initial revision.
 *
 *    Rev 1.0   Jun 03 2002 16:37:42   msg
 * Initial revision.
 *
 *    Rev 1.2   15 May 2002 17:14:04   vxs
 ...
 * Initial revision.
 *
 *    Rev 1.0   Sep 20 2001 15:58:24   msg
 * Initial revision.
 *
...
**/
//--------------------------------------------------------------------------
public class JdbcReadLayawayDescriptions
    extends JdbcReadFirstItemDescription
{
    /**
        The logger to which log messages will be sent.
    **/
    private static Logger logger = Logger.getLogger(oracle.retail.stores.domain.arts.JdbcReadLayawayDescriptions.class);
     ...
     **/
    //----------------------------------------------------------------------
    public JdbcReadLayawayDescriptions()
    {
        super();
        setName("JdbcReadLayawayDescriptions");
    }
    ...
    **/
                        ...
                        throws DataException
    {
        if (logger.isDebugEnabled()) logger.debug(
                     "JdbcReadLayawayDescriptions.execute");

        JdbcDataConnection connection = (JdbcDataConnection)dataConnection;

        LayawaySummaryEntryIfc[] summaryEntries;

        try
        {
            // This data operation should follow another data operation
            // that has retrieved the array of layaway summary entries.
            // Get the previously resulting array of layaway summary entries
            // from the data transaction.
            summaryEntries = (LayawaySummaryEntryIfc[])dataTransaction.getResult();

            // Loop through each entry and get the description for each
            for ( int x = 0; x < summaryEntries.length; x++)
            {
                ...
                TransactionSummaryIfc transactionSummary
                    = instantiateTransactionSummary(
                        summaryEntries[x].getInitialTransactionID(),
                        summaryEntries[x].getInitialTransactionBusinessDate(),
                        summaryEntries[x].getLocaleRequestor());
                ...
                // Call the inherited (from JdbcReadFirstItemDescription)
                // execute method with an appropriate data action.
                ...
                summaryEntries[x].setLocalizedDescriptions(descriptions);
            }

            // Set the result back to the original array of layaway summary
            // entries
            dataTransaction.setResult(summaryEntries);
        }
