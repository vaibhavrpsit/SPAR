 ...
 * ===========================================================================
 * $Header: rgbustores/modules/domain/src/oracle/retail/stores/domain/arts/JdbcRetrieveEMessagesByCustomer.java /rgbustores_13.4x_generic_branch/1 2011/05/04 11:49:04 mszekely Exp $
 * ===========================================================================
 * NOTES
 ...
 *:
 *    4    .v700     1.2.1.0     11/16/2005 16:26:23    Jason L. DeLeau 4215:
 ...
 * specified customer with optional storeid, begin date, and end date.
 * 
 * @version $Revision: /rgbustores_13.4x_generic_branch/1 $
 */
public class JdbcRetrieveEMessagesByCustomer extends JdbcDataOperation implements ARTSDatabaseIfc
{
    private static final long serialVersionUID = -1292162933668088817L;
    /**
     * The logger to which log messages will be sent.
     */
    private static final Logger logger = Logger.getLogger(JdbcRetrieveEMessagesByCustomer.class);

    /**
     * Class constructor.
     */
    public JdbcRetrieveEMessagesByCustomer()
    {
        super();
        setName("JdbcRetrieveEMessagesByCustomer");
    }
    ...
    {
        if (logger.isDebugEnabled()) logger.debug( "JdbcRetrieveEMessagesByCustomer.execute");

        // set data connection
        JdbcDataConnection connection = (JdbcDataConnection) dataConnection;

        // grab arguments and call RetrieveEMessagesByCustomer()
        OrderSearchKey orderSearchKey = (OrderSearchKey)action.getDataObject();
        Vector<EMessageIfc> emessageVector = RetrieveEMessagesByCustomer(connection, orderSearchKey);
        ...
        if (logger.isDebugEnabled())
            logger.debug("JdbcRetrieveEMessagesByCustomer.execute");
    }

    /**
     * Creates the SQL statements against the database. Retrieves emessage
     * objects based upon the orderSearchKey: customer,beginDate,endDate. The
     * beginDate, and endDate are optional.
     * 
     * @param dataConnection
     * @param orderSearchKey
     */
    public Vector<EMessageIfc> RetrieveEMessagesByCustomer(JdbcDataConnection connection,
                                              OrderSearchKey orderSearchKey) throws DataException
    {
        if (logger.isDebugEnabled()) logger.debug( "JdbcEMessageReadTransaction.RetrieveEMessagesByCustomer()");
        ...
        // get customer ID
        sql.addQualifier(FIELD_CUSTOMER_ID + " = '" + orderSearchKey.getCustomer().getCustomerID() + "'");
