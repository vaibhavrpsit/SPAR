 ...
 * ===========================================================================
 * $Header: rgbustores/modules/domain/src/oracle/retail/stores/domain/arts/JdbcAddStoreTotals.java /rgbustores_13.4x_generic_branch/1 2011/05/04 11:49:05 mszekely Exp $
 * ===========================================================================
 * NOTES
 ...
 *:
 *    4    .v700     1.2.1.0     11/16/2005 16:26:30    Jason L. DeLeau 4215:
 ...
 * Initial revision.
 *
 *    Rev 1.0   Jun 03 2002 16:35:16   msg
 * Initial revision.
 *
 ...
 * Initial revision.
 *
 *    Rev 1.0   Sep 20 2001 15:57:50   msg
 * Initial revision.
 *
...
/**
 * This operation modifies the Store History table
 * 
 * @version $Revision: /rgbustores_13.4x_generic_branch/1 $
 */
public class JdbcAddStoreTotals extends JdbcSaveStore
                                implements ARTSDatabaseIfc
{
    private static final long serialVersionUID = -2337544710982102951L;
    /**
        The logger to which log messages will be sent.
     */
    private static final Logger logger = Logger.getLogger(JdbcAddStoreTotals.class);

    /**
       Class constructor.
     */
    public JdbcAddStoreTotals()
    {
        super();
        setName("JdbcAddStoreTotals");
    }
    ...
    {
        if (logger.isDebugEnabled()) logger.debug( "JdbcAddStoreTotals.execute()");
        ...
        addStoreTotals(connection,
                       artsStore.getPosStore(),
                       artsStore.getBusinessDate(),
                       artsStore.getFinancialTotals());

        if (logger.isDebugEnabled()) logger.debug( "JdbcAddStoreTotals.execute()");
    }

    /**
       Adds the financial totals to the store history.
       <p>
       @param  dataConnection      The connection to the data source
       @param  store               The store to add the totals to
       @param  businessDate        The business date to add the totals to
       @param  totals              The financial totals information
       @exception DataException upon error
     */
    public void addStoreTotals(JdbcDataConnection dataConnection,
                               StoreIfc store,
                               EYSDate businessDate,
                               FinancialTotalsIfc totals)
        throws DataException
    {
        addStoreHistory(dataConnection, store, businessDate, totals);
    }
}
