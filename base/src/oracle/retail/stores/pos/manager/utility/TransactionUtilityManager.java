 ...
 *
 * MODIFIED    (MM/DD/YY)
 *    mjwallac  03/24/14 - fixed null derefs
 *    vineesin  03/12/14 - Throw tender limit error if transaction is
 *                         Tenderable Transaction
 *    kavdurai  02/18/14 - Fix for missing transaction in tr_trn table when
 *                         client closed abruptly after retrieval of suspended
 *                         transaction. Function added for archiving the
 *                         suspended retrieved transaction
 ...
 *    ckancher  08/16/13 - Fix to display currency type in Billpay transactions
 *    rgour     04/01/13 - CBR cleanup
 *    vbongu    02/12/13 - Use WriteHardTotalsCargoIfc instead of
 *                         AbstractFinancialCargoIfc in writeHardTotals(String)
 *                         method
 *    rgour     09/24/12 - setting Currency code and Country code for Cross
 *                         Border Return
 *    blarsen   04/06/12 - Added writeHardTotals method that accepts a filename
 *                         prefix (for mobile pos - needs to prefix w/
 *                         workstationID).
 *    cgreene   03/30/12 - Convert Hashtables and Vectors to Maps and Lists.
 *                         Cache tax rules by store id and lazily initialize
 *                         them if not set
 ...
 *    cgreene   03/15/12 - initial version
 *
 * ===========================================================================
 */
 ...
 * @since 13.4.1
 */
public class TransactionUtilityManager extends Manager
    implements TransactionUtilityManagerIfc
{
     ...
     *                created.
     */
    public TransactionUtilityManager()
    {
    }
