 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/modifytransaction/GiftReceiptAssignedAisle.java /main/12 2012/09/12 11:57:10 blarsen Exp $
 * ===========================================================================
 * NOTES
 ...
 *
 *   Revision 1.2  2004/08/23 16:15:57  cdb
...
public class GiftReceiptAssignedAisle extends PosLaneActionAdapter
{
    ...
    public void traverse(BusIfc bus)
    {
        ...
        ((SaleReturnTransactionIfc)cargo.getTransaction()).setTransactionGiftReceiptAssigned(true);

       //Set the Gift Receipt Flag
        for (int i=0; i < lineItems.length; i++)
        {
            // do not allow gift receipts for return items or damage discount items
            if(!((SaleReturnLineItemIfc)lineItems[i]).hasDamageDiscount() && ((SaleReturnLineItemIfc)lineItems[i]).isReturnLineItem() != true)
            {
                ((SaleReturnLineItemIfc)lineItems[i]).setGiftReceiptItem(true);
                itemID = lineItems[i].getItemID();

                //ITEM

                Object dataArgs[]={itemID.trim()};
                String item=I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.ITEM, dataArgs);

                sb.append(Util.EOL)
                  .append(item)
                  .append(Util.EOL)
                  .append(I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_GIFT_RECEIPT_LABEL, null));

            }

        } // End of items loop
        ((SaleReturnTransactionIfc)cargo.getTransaction()).setLineItems(lineItems);
        ...
        if (journal != null)
        {
            journal.journal(cargo.getSalesAssociate().getEmployeeID(),
