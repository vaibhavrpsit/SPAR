 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/tender/authorization/referral/SendCallReferralAuthorizationSite.java /main/6 2014/07/01 13:33:27 blarsen Exp $
 * ===========================================================================
 * NOTES
 ...
 *    blarsen   04/30/13 - Saving the original auth's raw response. Needed by
 *                         AJB's call referral SAF request.
 *    tksharma  12/10/12 - commons-lang update 3.1
 *    asinton   07/25/12 - Dont send a call referral unless the tender type is
 *                         credit.
...
public class SendCallReferralAuthorizationSite extends PosSiteActionAdapter
{
    ...
    public void arrive(BusIfc bus)
    {
        ...
        if(response != null)
        {
            ...
            // for some authorization services, the call referral request (sent to the auth service) must contain detailed information
            // contained in the original auth response from the service
            callReferralRequest.setRawResponse(response.getRawResponse());

            boolean callPaymentManager = false;
            RequestType requestType = request.getRequestType();
            // credit, debit
            if(PaymentServiceRequestIfc.RequestType.AuthorizeCard.equals(requestType) && TenderType.CREDIT.equals(response.getTenderType()))
            {
            ...
            if(callPaymentManager)
            {
                ...
                try
                {
                    ...
                    // if call referral then update to the transaction archive
                    ...
                    helper.addPendingAuthorizationToTransactionArchive(bus, request, callReferralResponse);
                }
                catch(Exception e)
                {
